...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if torch.rand(1) < 0.5:
        xb = T.functional.hflip(xb)
    xb = T.functional.adjust_brightness(xb, 0.8 + 0.4 * torch.rand(1))
    xb = T.functional.adjust_contrast(xb, 0.8 + 0.4 * torch.rand(1))
    return xb
...
def main() -> int:
    ...
    total_steps = steps_per_epoch * 10  # Estimate based on 10 epochs
    ...
    current_step = steps
    cosine_lr = LEARNING_RATE * 0.5 * (1 + math.cos(math.pi * current_step / total_steps))
