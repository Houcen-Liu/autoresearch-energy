...
def build_model() -> nn.Module:
    ...
    model.add_module("norm2", nn.BatchNorm2d(32))
    model.add_module("norm3", nn.BatchNorm2d(64))
    model.add_module("norm4", nn.BatchNorm2d(64))
    model.add_module("norm5", nn.BatchNorm2d(128))
...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Apply normalization using known CIFAR-10 statistics
    mean = torch.tensor([0.4914, 0.4822, 0.4465]).view(1, 3, 1, 1).to(xb.device)
    std = torch.tensor([0.2470, 0.2435, 0.2616]).view(1, 3, 1, 1).to(xb.device)
    return (xb - mean) / std
