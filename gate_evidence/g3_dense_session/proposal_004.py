...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    xb = transforms.functional.adjust_brightness(xb, 0.8 + 0.4 * torch.rand(1))
    xb = transforms.functional.adjust_contrast(xb, 0.8 + 0.4 * torch.rand(1))
    xb = transforms.functional.adjust_saturation(xb, 0.8 + 0.4 * torch.rand(1))
    return xb
