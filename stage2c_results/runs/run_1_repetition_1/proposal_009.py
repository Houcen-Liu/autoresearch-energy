...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if torch.rand(1) < 0.5:
        xb = torch.flip(xb, [3])  # Random horizontal flip
    # Color jitter
    brightness = torch.rand(1) * 0.4 + 0.8
    contrast = torch.rand(1) * 0.4 + 0.8
    saturation = torch.rand(1) * 0.4 + 0.8
    hue = torch.rand(1) * 0.1
    xb = torchvision.transforms.functional.adjust_brightness(xb, brightness)
    xb = torchvision.transforms.functional.adjust_contrast(xb, contrast)
    xb = torchvision.transforms.functional.adjust_saturation(xb, saturation)
    xb = torchvision.transforms.functional.adjust_hue(xb, hue)
    return xb
