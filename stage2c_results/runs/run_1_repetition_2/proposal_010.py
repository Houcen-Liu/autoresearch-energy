...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Add random horizontal flips and color jitter
    if torch.rand(1) < 0.5:
        xb = torch.flip(xb, [3])
    # Add color jitter
    xb = xb * torch.tensor([0.2, 0.2, 0.2], device=xb.device, dtype=xb.dtype).view(3, 1, 1, 1) + \
         torch.tensor([0.4, 0.4, 0.4], device=xb.device, dtype=xb.dtype).view(3, 1, 1, 1)
    return xb
