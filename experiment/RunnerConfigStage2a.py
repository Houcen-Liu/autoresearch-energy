...
STAGE 2a (reasoning): 2 proposers x 2 thinking modes x 3 reps = 12 sessions.

NAMING. The proposal defines two orthogonal axes whose names are easy to
conflate. STAGES split the research question: Stage 1 fixes the intra-model
configuration and varies architecture (the 2x2x2 factorial); Stage 2 varies the
intra-model configuration itself. PHASES split Stage 1 by hardware: Phase 1 is
the GPU factorial, Phase 2 is CPU-only proposer serving.

Reasoning mode is an intra-model setting, listed among Stage 1's fixed
variables, so varying it is Stage 2 work -- not the proposal's Phase 2, which is
the CPU study. This file is Stage 2a; the temperature sweep is Stage 2b.
...
Stage 2a promotes it to a factor against that clean baseline.
...
# Fixed variables for Stage 2a, taken from the cheapest Phase-1 configuration.
STAGE2A_PATIENCE = 1
STAGE2A_LOOP_BUDGET = 10


class RunnerConfig:
    ROOT_DIR = Path(__file__).resolve().parent

    name: str = "autoresearch_energy_stage2a_reasoning"
    ...
    def create_run_table_model(self) -> RunTableModel:
        ...
        self.run_table_model = RunTableModel(
            ...
            data_columns=[
                ...
                # Stage-2a specific. thinking_tokens_total is the manipulation
    ...
    def start_run(self, context: RunnerContext) -> None:
        ...
        cmd = [sys.executable, "-m", "harness.agent_loop",
               ...
               # fixed in Stage 2a -- see the module docstring
               "--patience", str(STAGE2A_PATIENCE),
               "--loop-budget", str(STAGE2A_LOOP_BUDGET),
    ...
    def interact(self, context: RunnerContext) -> None:
        ...
        # loop_budget is fixed in Stage 2a, not a run-table factor. Reasoning
        ...
        expected = STAGE2A_LOOP_BUDGET * per_iter + 900   # + model swap
    ...
    def populate_run_data(self, context: RunnerContext) -> Optional[Dict[str, Any]]:
        ...
        # MANIPULATION CHECK. The whole of Stage 2a rests on the serving stack
