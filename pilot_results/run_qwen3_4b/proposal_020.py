...
def build_model() -> nn.Module:
    ...
    return nn.Sequential(
        ...
        nn.ReLU(),
        nn.Dropout(p=0.7),  # Increased dropout rate to 0.7 for better generalization
        nn.Linear(256, 10),
    )
...
def main() -> int:
    ...
    steps = epochs = 0
    model.train()
    t0 = time.monotonic()                        # <-- Training starts here

    for _ in range(10):  # Simulate 10 epochs for demonstration
