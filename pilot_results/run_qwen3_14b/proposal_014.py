...
def build_model() -> nn.Module:
    ...
    return nn.Sequential(
        ...
        nn.MaxPool2d(2), nn.Dropout2d(0.25),  # 8x8
        nn.Conv2d(64, 128, 3, padding=1), nn.BatchNorm2d(128), nn.ReLU(),
        nn.MaxPool2d(2), nn.Dropout2d(0.25),  # 4x4
        nn.Flatten(),
...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if random.random() > 0.5:
        xb = TF.hflip(xb)
    # Color jitter
    xb = TF.adjust_brightness(xb, 1.2)
    xb = TF.adjust_contrast(xb, 1.2)
    # Random erasing
    if random.random() > 0.5:
        xb = TF.erase(xb, 0.1, 0.3, 0.3, 0.7, 0.5, 0.5, 0.5)
    return xb
