...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    transform = T.RandomApply([T.ColorJitter(0.6, 0.6, 0.6, 0.2)], p=0.9)  # Increased intensity
    xb = transform(xb)
    transform = T.RandomApply([T.RandomCrop(32, padding=4)], p=1.0)  # Always apply crop
    xb = transform(xb)
    transform = T.RandomErasing(p=0.8, scale=(0.05, 0.33), ratio=(0.3, 3.3), value=0.0)  # Higher erasing probability
    xb = transform(xb)
    return xb
...
def main() -> int:
    ...
    opt = torch.optim.SGD(model.parameters(), lr=LEARNING_RATE, momentum=0.9, weight_decay=WEIGHT_DECAY)  # Switched to SGD
