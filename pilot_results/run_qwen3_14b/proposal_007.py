...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    padding = 4
    xb = F.pad(xb, (padding, padding, padding, padding), mode='reflect')
    xb = F.interpolate(xb, scale_factor=1.2, mode='bilinear', align_corners=False)
    xb = F.pad(xb, (0, 0, 0, 0), mode='reflect')
    xb = F.interpolate(xb, scale_factor=1/1.2, mode='bilinear', align_corners=False)
