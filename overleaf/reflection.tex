...
\textbf{The first energy measurement of an agentic AutoML loop.} Published
...
cost but not energy. I could find no prior study that measures the energy of a
closed propose--evaluate--keep loop, where proposal quality determines how much
downstream training compute is spent. The specific finding, that a sparse
proposer is simultaneously cheaper and more accurate on this workload, so that
the expected sparsity trade-off does not appear, is therefore new, though it
is one workload and one model pair.
...
means $E_{\mathit{prop}}$ and $E_{\mathit{train}}$ are read from different
devices instead of being separated by a calibration model. The contribution is
less the idea than its verification: the alignment gate confirms empirically
that the training device draws more during training and the proposer device
during proposing, rather than assuming the indices are wired correctly. That
check caught nothing in the end, but a study that omits it cannot say so.
...
loop degenerate, and the experiment I ran to support it refuted it instead. What
survives is narrower: temperature governs how often the proposer repeats itself
and not what the session achieves.
...
identical. It was still wrong. A later sweep showed temperature 0 retains
mutations at the normal rate, and that three sessions retaining nothing is an
ordinary result for the dense arm, which retains nothing 42\,\% of the time. The
...
the mechanism I proposed does not operate, not what the prompt contributed on the
dense arm. Twelve sessions were spent recovering an answer that a
one-change-at-a-time intervention would have given for free.

I also under-powered accuracy relative to energy. An unplanned replication,
the same configuration executed in two experiments days apart, reproduced
session energy to 0.20\,\% and test accuracy to only 6.42\,pp. Given that
