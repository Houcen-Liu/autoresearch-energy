...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Stronger random crop, horizontal flip, and cutmix
    if torch.rand(1) < 0.5:
        # Random crop
        padding = 4
        batch_size, channels, height, width = xb.shape
        pad = F.pad(xb, (padding, padding, padding, padding), mode='replicate')
        # Random crop
        offset_x = torch.randint(0, padding * 2, (1,))
        offset_y = torch.randint(0, padding * 2, (1,))
        cropped = pad[:, :, offset_x:offset_x + height, offset_y:offset_y + width]
        return cropped
    elif torch.rand(1) < 0.2:
        # Cutmix
        alpha = 1.0
        lam = np.random.beta(alpha, alpha)
        rand_index = torch.randperm(xb.size()[0])
        bbx1, bby1, bbx2, bby2 = rand_bbox(xb.size(), lam)
        xb[:, :, bbx1:bbx2, bby1:bby2] = xb[rand_index][:, :, bbx1:bbx2, bby1:bby2]
    return xb


def rand_bbox(xb: torch.Tensor, lam: float) -> tuple[int, int, int, int]:
    """Random Bounding Box for cutmix."""
    B, C, H, W = xb.size()
    cut_ratio = 1. - lam
    cut_h = int(H * torch.rand(1).item())
    cut_w = int(W * torch.rand(1).item())
    offset_h = torch.randint(0, H - cut_h, (1,)).item()
    offset_w = torch.randint(0, W - cut_w, (1,)).item()
    return offset_h, offset_w, cut_h, cut_w
