...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Apply random horizontal flips, color jitter, and random erasing
    if torch.rand(1) < 0.5:
        xb = torch.flip(xb, [3])  # Horizontal flip
    # Color jitter
    xb = xb.permute(0, 2, 3, 1)  # NHWC
    mean = xb.mean(dim=(1, 2, 3), keepdim=True)
    std = xb.std(dim=(1, 2, 3), keepdim=True)
    xb = (xb - mean) / std
    # Random color jitter
    if torch.rand(1) < 0.5:
        xb = xb * torch.rand(1, 3, 1, 1) + torch.rand(1, 3, 1, 1)
    # Random erasing
    if torch.rand(1) < 0.2:
        ...
        for _ in range(2):
            ...
            xb[:, y:y+int(h/2), x:x+int(w/2), :] = 0
    xb = xb.permute(0, 3, 1, 2)  # NCHW
