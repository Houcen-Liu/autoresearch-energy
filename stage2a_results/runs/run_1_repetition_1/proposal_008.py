...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Apply random horizontal flips, color jitter, and random crops
    if torch.rand(1) < 0.5:
        xb = torch.flip(xb, [3])  # Horizontal flip
    # Color jitter
    if torch.rand(1) < 0.3:
        xb = xb * torch.rand(3, 1, 1, 1, device=xb.device) + torch.rand(3, 1, 1, 1, device=xb.device)
    # Random crop with padding
    pad = 4
    xb = F.pad(xb, (pad, pad, pad, pad))
    i = torch.randint(0, 8, (1,))  # 0-7 to ensure crop fits
