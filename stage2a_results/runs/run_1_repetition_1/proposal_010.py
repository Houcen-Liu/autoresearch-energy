...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Apply random horizontal flips, random crops, color jitter, and cutout
    if torch.rand(1) < 0.5:
    ...
    xb = xb[:, :, i:i+32, j:j+32]
    # Color jitter
    xb = xb + torch.rand(3, 1, 1, 1, device=xb.device) * 0.2 - 0.1
    # Cutout
    if torch.rand(1) < 0.2:
        mask = torch.rand(1, 1, 32, 32, device=xb.device) < 0.3
        xb = xb * (~mask).float()
    return xb
