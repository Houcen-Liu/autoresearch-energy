...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if torch.rand(1) < 0.2:
        xb = xb * torch.rand(3, 1, 1, device=xb.device) + torch.rand(3, 1, 1, device=xb.device)
    if torch.rand(1) < 0.1:
        size = 10
        i = torch.randint(0, 32 - size, (1,))
        j = torch.randint(0, 32 - size, (1,))
        xb[:, :, i:i+size, j:j+size] = torch.rand(3, 1, 1, device=xb.device) * 0.5
    return xb
