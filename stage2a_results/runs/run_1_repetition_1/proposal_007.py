...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Apply random horizontal flips, random crops, and color augmentations
    if torch.rand(1) < 0.5:
        xb = torch.flip(xb, [3])  # Horizontal flip
    # Random crop with padding
    pad = 4
    xb = F.pad(xb, (pad, pad, pad, pad))
    i = torch.randint(0, 2 * pad, (1,))
    j = torch.randint(0, 2 * pad, (1,))
    xb = xb[:, :, i:i+32, j:j+32]
    
    # Color jitter
    if torch.rand(1) < 0.5:
        xb = xb + 0.2 * torch.rand(1, 3, 32, 32, device=xb.device) - 0.1
    # Random erasing
    if torch.rand(1) < 0.2:
        h, w = xb.shape[2], xb.shape[3]
        x1, y1 = torch.randint(0, h, (1,)), torch.randint(0, w, (1,))
        x2, y2 = torch.randint(x1+1, h, (1,)), torch.randint(y1+1, w, (1,))
        xb[:, :, x1:x2, y1:y2] = 0.5 * torch.rand(1, 3, x2-x1, y2-y1, device=xb.device)
    return xb
