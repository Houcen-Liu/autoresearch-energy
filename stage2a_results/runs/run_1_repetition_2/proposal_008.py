...
def build_model() -> nn.Module:
    ...
    return nn.Sequential(
        nn.Conv2d(3, 32, 3, padding=1), nn.ReLU(), nn.BatchNorm2d(32), nn.Dropout2d(0.25),
        nn.Conv2d(32, 32, 3, padding=1), nn.ReLU(), nn.BatchNorm2d(32),
        nn.MaxPool2d(2),                                    # 16x16
        nn.Conv2d(32, 64, 3, padding=1), nn.ReLU(), nn.BatchNorm2d(64), nn.Dropout2d(0.25),
        nn.Conv2d(64, 64, 3, padding=1), nn.ReLU(), nn.BatchNorm2d(64),
        nn.MaxPool2d(2),                                    # 8x8
        nn.Conv2d(64, 128, 3, padding=1), nn.ReLU(), nn.BatchNorm2d(128), nn.Dropout2d(0.25),
        nn.MaxPool2d(2),                                    # 4x4
        nn.Flatten(),
...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    transform = transforms.Compose([
        ...
        transforms.RandomApply([transforms.CutOut(size=16)], p=0.5),
    ])
    return transform(xb)
