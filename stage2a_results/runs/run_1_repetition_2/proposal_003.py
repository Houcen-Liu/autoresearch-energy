...
def main() -> int:
    ...
    x_tr = torch.from_numpy(d["x_train"]).to(device)  # Ensure data is on the same device as model
    y_tr = torch.from_numpy(d["y_train"]).to(device)
    x_va = torch.from_numpy(d["x_val"]).to(device)
    y_va = torch.from_numpy(d["y_val"]).to(device)
    ...
    xb0 = x_tr[:BATCH_SIZE]
    F.cross_entropy(model(xb0), y_tr[:BATCH_SIZE]).backward()
    ...
    while time.monotonic() - t0 < budget:
        ...
        for i in range(steps_per_epoch):
            ...
            xb = augment(x_tr[idx])  # xb is already on the same device as model
            yb = y_tr[idx]         # yb is already on the same device as model
