...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if torch.rand(1).item() < 0.5:
        # Randomly select a batch to mix with
        idx = torch.randperm(xb.size(0))
        lam = torch.rand(xb.size(0), 1, 1, 1).to(xb.device)
...
def main() -> int:
    ...
    scheduler = torch.optim.lr_scheduler.SequentialLR(
        opt, 
        [
            torch.optim.lr_scheduler.LinearLR(opt, start_factor=0.1, total_iters=5),
            torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=20)
        ],
        verbose=False
    )
