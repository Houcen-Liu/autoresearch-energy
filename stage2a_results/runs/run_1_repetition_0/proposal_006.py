...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if random.random() < 0.5:
        xb = transforms.functional.pad(xb, padding=4, fill=0)
        xb = transforms.functional.center_crop(xb, [32, 32])
    # Color jitter
    if random.random() < 0.5:
        xb = transforms.functional.adjust_brightness(xb, brightness_factor=1.2)
        xb = transforms.functional.adjust_contrast(xb, contrast_factor=1.2)
        xb = transforms.functional.adjust_saturation(xb, saturation_factor=1.2)
    return xb
...
def main() -> int:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=steps_per_epoch * 10)
