...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if random.random() < 0.5:
        ...
        xb = transforms.functional.pad(xb, 4, fill=0)
        i, j, h, w = transforms.RandomCrop(32, padding=4).get_params(xb, (32, 32))
...
def main() -> int:
    ...
    scheduler = lr_scheduler.CosineAnnealingLR(optimizer=opt, T_max=steps_per_epoch * 4)
