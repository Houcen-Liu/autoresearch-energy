...
def build_model() -> nn.Module:
    ...
    return nn.Sequential(
        nn.Conv2d(3, 32, 3, padding=1), nn.BatchNorm2d(32), nn.ReLU(), nn.Dropout(0.2),
        nn.Conv2d(32, 32, 3, padding=1), nn.BatchNorm2d(32), nn.ReLU(), nn.Dropout(0.2),
        nn.MaxPool2d(2),                                    # 16x16
        nn.Conv2d(32, 64, 3, padding=1), nn.BatchNorm2d(64), nn.ReLU(), nn.Dropout(0.2),
        nn.Conv2d(64, 64, 3, padding=1), nn.BatchNorm2d(64), nn.ReLU(), nn.Dropout(0.2),
        nn.MaxPool2d(2),                                    # 8x8
        nn.Conv2d(64, 128, 3, padding=1), nn.BatchNorm2d(128), nn.ReLU(), nn.Dropout(0.2),
        nn.MaxPool2d(2),                                    # 4x4
        nn.Flatten(),
        nn.Linear(128 * 4 * 4, 256), nn.ReLU(), nn.Dropout(0.2),
        nn.Linear(256, 10),
    )
...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if random.random() < 0.5:
        xb = transforms.functional.hflip(xb)
    # Random crop with padding
    padding = 4
    i, j, h, w = transforms.RandomCrop(32, padding=padding).get_params(xb, (32, 32))
    xb = transforms.functional.crop(xb, i, j, h, w)
    # Color jitter
    xb = transforms.functional.adjust_brightness(xb, 1.2)
    xb = transforms.functional.adjust_contrast(xb, 1.2)
    xb = transforms.functional.adjust_saturation(xb, 1.2)
    xb = transforms.functional.adjust_hue(xb, 0.1)
    return xb
...
def main() -> int:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=opt, T_max=43 * 500)
