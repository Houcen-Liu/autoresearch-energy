...
def build_model() -> nn.Module:
    ...
    return nn.Sequential(
        nn.Conv2d(3, 32, 3, padding=1), nn.BatchNorm2d(32), nn.ReLU(), nn.Dropout2d(0.2),
        nn.Conv2d(32, 32, 3, padding=1), nn.BatchNorm2d(32), nn.ReLU(), nn.Dropout2d(0.2),
        nn.MaxPool2d(2),                                    # 16x16
        nn.Conv2d(32, 64, 3, padding=1), nn.BatchNorm2d(64), nn.ReLU(), nn.Dropout2d(0.2),
        nn.Conv2d(64, 64, 3, padding=1), nn.BatchNorm2d(64), nn.ReLU(), nn.Dropout2d(0.2),
        nn.MaxPool2d(2),                                    # 8x8
        nn.Conv2d(64, 128, 3, padding=1), nn.BatchNorm2d(128), nn.ReLU(), nn.Dropout2d(0.2),
        nn.MaxPool2d(2),                                    # 4x4
        nn.Flatten(),
...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if random.random() < 0.5:
        xb = transforms.functional.pad(xb, 4, mode='reflect')
        xb = transforms.functional.center_crop(xb, [32, 32])
    # Color jitter
    if random.random() < 0.5:
        xb = transforms.functional.adjust_brightness(xb, 0.8 + 0.4 * random.random())
        xb = transforms.functional.adjust_contrast(xb, 0.8 + 0.4 * random.random())
        xb = transforms.functional.adjust_saturation(xb, 0.8 + 0.4 * random.random())
    return xb
...
def main() -> int:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=opt, T_max=100)
