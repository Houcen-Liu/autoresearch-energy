...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if torch.rand(1).item() > 0.5:
        pad = 4
        xb = F.pad(xb, (pad, pad, pad, pad), mode='constant', value=0)
        # Randomly select a crop
        h = torch.randint(0, 4, (xb.size(0), 1, 1, 1))
        w = torch.randint(0, 4, (xb.size(0), 1, 1, 1))
        xb = xb[:, :, h:h+32, w:w+32]
    return xb
...
def main() -> int:
    ...
    scheduler = lr_scheduler.ReduceLROnPlateau(opt, 'max', patience=2, verbose=True)
    ...
    while time.monotonic() - t0 < budget:
        ...
        for i in range(steps_per_epoch):
            ...
            epochs += 1
            # Evaluate on validation set to adjust LR
