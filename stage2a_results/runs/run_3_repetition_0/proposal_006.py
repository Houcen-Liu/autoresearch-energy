...
def augment(xb: torch.Tensor) -> torch.Tensor:
    """Data augmentation with random cropping/padding and horizontal flip."""
    ...
    if torch.rand(1).item() > 0.5:
        ...
        # Random crop
        offset_h = torch.randint(0, padding*2, (1,)).to(xb.device)
        offset_w = torch.randint(0, padding*2, (1,)).to(xb.device)
        xb = xb[:, :, offset_h:offset_h + 32, offset_w:offset_w + 32]
