...
by harness/guards.py before this file is ever executed:
  ...
  * Data comes only from prepare_cifar.load_splits(); the test split is off limit.
...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if torch.rand(1).item() > 0.5:
        xb = torch.flip(xb, [3])  # flip along width (channel is 1, height is 2, width is 3)
    
    # Random cropping with padding
    padding = 4
    xb = F.pad(xb, (padding, padding, padding, padding), "constant", 0)
    
    # Random crop
    crop_size = 32
    offset_h = torch.randint(0, padding*2, (xb.size(0), 1, 1, 1))
    offset_w = torch.randint(0, padding*2, (xb.size(0), 1, 1, 1))
    xb = xb.gather(2, offset_h.expand(-1, -1, crop_size, -1)).gather(3, offset_w.expand(-1, -1, -1, crop_size))
    return xb
...
def main() -> int:
    ...
    scheduler = lr_scheduler.CosineAnnealingLR(opt, T_max=45, eta_min=1e-5)
