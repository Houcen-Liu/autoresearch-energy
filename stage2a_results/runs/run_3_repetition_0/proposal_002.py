...
def main() -> int:
    ...
    x_tr = torch.from_numpy(d["x_train"]).to(device, non_blocking=True)
    y_tr = torch.from_numpy(d["y_train"]).to(device, non_blocking=True)
    x_va = torch.from_numpy(d["x_val"]).to(device, non_blocking=True)
    y_va = torch.from_numpy(d["y_val"]).to(device, non_blocking=True)
    ...
    xb0 = x_tr[:BATCH_SIZE]
    loss = F.cross_entropy(model(xb0), y_tr[:BATCH_SIZE])
    loss.backward()
