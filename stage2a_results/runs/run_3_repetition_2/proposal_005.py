...
def augment(xb: torch.Tensor) -> torch.Tensor:
    """Data augmentation with random crop, flip, rotation, and color jitter."""
    ...
    angle = torch.rand(1) * 10 - 5
    theta = torch.tensor([[1, 0, 0], [0, 1, 0]], device=xb.device).unsqueeze(0)
    theta = theta.expand(xb.size(0), -1, -1)
    grid = F.affine_grid(theta, xb.size(), align_corners=False)
    xb = F.grid_sample(xb, grid, mode='bilinear', align_corners=False)
    # Color jitter
    brightness = torch.rand(1) * 0.2 + 0.8
    contrast = torch.rand(1) * 0.2 + 0.8
    saturation = torch.rand(1) * 0.2 + 0.8
    xb = T.functional.adjust_brightness(xb, brightness)
    xb = T.functional.adjust_contrast(xb, contrast)
    xb = T.functional.adjust_saturation(xb, saturation)
    return xb
...
def main() -> int:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=43, eta_min=1e-4)
