...
def augment(xb: torch.Tensor) -> torch.Tensor:
    """Data augmentation with random crop, horizontal flip, and color jitter."""
    ...
    # Color jitter
    if torch.rand(1) < 0.5:
        xb = F.adjust_brightness(xb, 1.2)
        xb = F.adjust_contrast(xb, 1.2)
        xb = F.adjust_saturation(xb, 1.2)
    return xb
...
def main() -> int:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=10, eta_min=1e-4)
