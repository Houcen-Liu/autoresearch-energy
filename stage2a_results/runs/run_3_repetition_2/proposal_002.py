...
def augment(xb: torch.Tensor) -> torch.Tensor:
    """Data augmentation with random crop, flip, and color jitter."""
    ...
    # Color jitter
    if torch.rand(1) < 0.5:
        xb = T.functional.adjust_brightness(xb, 1.2)
        xb = T.functional.adjust_contrast(xb, 1.2)
    return xb
...
def main() -> int:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=43, eta_min=1e-6)
