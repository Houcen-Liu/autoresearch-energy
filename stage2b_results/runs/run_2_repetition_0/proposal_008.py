...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Add random horizontal flipping and cropping
    if torch.rand(1) < 0.5:
        # For mixup, we need to generate a random lambda and select a random
        # batch of images to mix with
        lam = torch.rand(xb.shape[0], 1, 1, 1).to(xb.device)
        index = torch.randperm(xb.size(0)).to(xb.device)
        xb = lam * xb + (1 - lam) * xb[index]
    if torch.rand(1) < 0.5:
        xb = F.interpolate(xb, size=36, mode='bilinear', align_corners=False)
        xb = F.pad(xb, (2, 2, 2, 2))
        xb = F.interpolate(xb, size=32, mode='bilinear', align_corners=False)
    if torch.rand(1) < 0.5:
...
def main() -> int:
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(opt, milestones=[15, 30], gamma=0.1)
