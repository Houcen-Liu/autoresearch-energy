...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if torch.rand(1) < 0.5:
        # Random crop
        pad = 4
        batch_size, c, h, w = xb.shape
        offset_h = torch.randint(0, pad*2, (1,), device=xb.device)
        offset_w = torch.randint(0, pad*2, (1,), device=xb.device)
        cropped = F.pad(xb, (offset_w, pad*2 - offset_w, offset_h, pad*2 - offset_h))
        return cropped[:, :, offset_h:offset_h+h, offset_w:offset_w+w]
    return xb
