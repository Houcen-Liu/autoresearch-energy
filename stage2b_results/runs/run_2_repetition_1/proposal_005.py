...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Add random horizontal flips and random crops
    if torch.rand(1) < 0.5:
        xb = F.pad(xb, (4, 4, 4, 4), padding_mode='reflect')
        indices = torch.randperm(xb.size(2) * xb.size(3))
        xb = xb[:, :, indices[:xb.size(2)*xb.size(3]//2].view(xb.size(0), xb.size(1), xb.size(2), xb.size(3))
    return xb
