...
by harness/guards.py before this file is ever executed:
  ...
  * build_model() must exist and return the trained model, so final_eval.py can rebuild it.
...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Simple random horizontal flip and random crop with padding
    if torch.rand(1) < 0.5:
        xb = torch.flip(xb, dims=[3])
    if torch.rand(1) < 0.5:
        padding = 4
        xb = F.pad(xb, (padding, padding, padding, padding), mode='constant', value=0)
    ...
    if torch.rand(1) < 0.5:
        ...
        index = torch.randperm(batch_size).to(xb.device)
...
def main() -> int:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=20, eta_min=0.001)
