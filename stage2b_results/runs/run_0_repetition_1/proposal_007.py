...
def main() -> int:
    ...
    with torch.amp.autocast("cuda", dtype=torch.float16) if MIXED_PRECISION else torch.no_grad():
        loss = criterion(model(xb0), y_tr[:BATCH_SIZE].to(device))
    loss.backward()
