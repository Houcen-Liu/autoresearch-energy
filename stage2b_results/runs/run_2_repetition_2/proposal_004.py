...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Simple random horizontal flip and crop
    if torch.rand(1) < 0.5:
        xb = xb.flip(dims=(-1,))
    if torch.rand(1) < 0.5:
        pad = torch.randint(2, 4, (1,))
        xb = F.pad(xb, (pad[0], pad[0], pad[0], pad[0]), "constant", 0)
...
def main() -> int:
    ...
    def get_lr(step, warmup_steps=10):
        if step < warmup_steps:
            return 0.1 * (step + 1) / warmup_steps
        return 0.1 * (1 + math.cos(math.pi * (step - warmup_steps) / (20 - warmup_steps))) / 2
