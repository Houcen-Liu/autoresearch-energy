...
week 1 to land in the 0.60-0.85 validation band (gate G2). Once calibrated it is
a FIXED VARIABLE: pin the commit and never touch it again.
"""
...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    angle = torch.tensor(15.0 * (2 * torch.rand(1) - 1.0))
    return TF.rotate(xb, angle.item(), interpolation=TF.InterpolationMode.BILINEAR)
