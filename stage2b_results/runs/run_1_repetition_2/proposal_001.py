...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    # Simple random horizontal flip and rotation
    if torch.rand(1).item() > 0.5:
        xb = torch.flip(xb, dims=[3])  # Horizontal flip
    if torch.rand(1).item() > 0.3:
        angle = torch.rand(1).item() * 10 - 5  # Random rotation between -5 and 5 degrees
        xb = F.interpolate(xb, size=(32, 32), mode='bilinear', align_corners=False)
        xb = F.rotate(xb, angle, interpolation='bilinear')
    return xb
