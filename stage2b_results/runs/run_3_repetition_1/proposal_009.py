...
def main() -> int:
    ...
    # Add label smoothing and stochastic depth
    criterion = torch.nn.CrossEntropyLoss(label_smoothing=0.1)
    # Stochastic depth rate: 0.2 at start, decays to 0 over 20 epochs
    # This is a simple implementation - more sophisticated approaches are possible
    def stochastic_depth_rate(epoch):
        return 0.2 * (1 - epoch / 20)
    ...
    while time.monotonic() - t0 < budget:
        ...
        for i in range(steps_per_epoch):
            ...
            scheduler.step()
            steps += 1
        # Update stochastic depth rate after each epoch
        if epochs < 20:
            for module in model.modules():
                if isinstance(module, nn.Dropout):
                    module.p = stochastic_depth_rate(epochs)
        epochs += 1
