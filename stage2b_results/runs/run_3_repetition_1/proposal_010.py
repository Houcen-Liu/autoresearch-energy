...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if torch.rand(1).item() > 0.5:
        ...
        bbx1, bby1, bbw, bbh = [int(torch.rand(1).item() * 32) for _ in range(4)]
        xb[:, :, bby1:bby1+bbh, bbx1:bbx1+bbw] = xb[rand_idx][:, :, bby1:bby1+bbh, bbx1:bbx1+bbw] * lambda_
    return xb
...
def main() -> int:
    ...
    # Add label smoothing and mixup
    criterion = torch.nn.CrossEntropyLoss(label_smoothing=0.1)

    # Learning rate warmup
    warmup_steps = 5
    lr_schedule = torch.optim.lr_scheduler.LinearWarmup(
        opt, 
        end_factor=1.0, 
