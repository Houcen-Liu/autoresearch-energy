...
def build_model() -> nn.Module:
    ...
    return nn.Sequential(
        ...
        nn.Dropout(p=0.5),  # Stochastic depth regularization
        nn.Linear(256, 10),
    )
...
def augment(xb: torch.Tensor) -> torch.Tensor:
    ...
    if torch.rand(1).item() < 0.5:
        xb = torch.flip(xb, dims=[3])  # Random horizontal flip
    if torch.rand(1).item() < 0.5:
...
def main() -> int:
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(opt, T_max=45, eta_min=0.001)
