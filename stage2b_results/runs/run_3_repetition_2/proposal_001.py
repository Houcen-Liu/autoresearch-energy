...
# ---------------------------------------------------------------- fixed budget
TRAIN_SECONDS = 45.0          # DO NOT MODIFY -- enforced by harness/guards.py
...
def augment(xb: torch.Tensor) -> torch.Tensor:
    """Data augmentation with random horizontal flipping and padding."""
    ...
    padded = F.pad(xb, (padding, padding, padding, padding), "constant", 0)
    
    # Random crop
    offset_x = torch.randint(0, padding*2, (1,), device=xb.device)
    offset_y = torch.randint(0, padding*2, (1,), device=xb.device)
    cropped = padded[:, :, offset_x:offset_x+height, offset_y:offset_y+width]
